...
const ModalPortalHost = (props) => {
    const { hasHostId, addHostId, removeHostId } = (0, react_1.use)(exports.PortalContext);
    (0, react_1.useEffect)(() => {
        if (hasHostId(props.hostId)) {
            throw new Error(`ModalPortalHost with hostId "${props.hostId}" already exists. Each hostId must be unique.`);
        }
    ...
    return (<native_1.NativeModalPortalHost style={{ flex: props.fluid ? 1 : undefined }} fluid={props.fluid} hostId={props.hostId}/>);
};
